...
#include "Default_Envelope.h"       //pre-loaded envelope
...
const int MAX_POINTS = 240;  // maximun data EG_form lenght for storage. It's not the screen height!!
...
int relPosX = map(relDrawX, 0, SCREEN_WIDTH, 0, 4095);
int resIndex = 13;//touch data acquisition space interval.

void setup() {
  // Display init
  tft = new Adafruit_ST7789(TFT_CS_PIN, TFT_DC_PIN, TFT_RST_PIN);
  tft->init(SCREEN_HEIGHT, SCREEN_WIDTH);   // standard init ST7789, portrait 240x320
...
void TFT_DRAW_LINES(){
  tft->drawFastVLine(relDrawX, 0, SCREEN_HEIGHT, WHITE); //x0, y0, lenght, color
  //tft->drawFastHLine(0, 23, SCREEN_WIDTH, WHITE); //x0, y0, lenght, color
}
